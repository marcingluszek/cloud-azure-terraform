...
# publiczny IP typy Dynamic powstaje, gdy podlaczona do niego VM-ka zostanie uruchomiona
...
resource "azurerm_network_security_group" "hub-vm-nsg" {
    ...
    security_rule {
        ...
        source_address_prefix      = "*" #dopisac mój adres ip
        destination_address_prefix = "*" #dopisać adres maszynki jaka powstała czyli public adrs ip
    }
}

# Create network interface for hub vm ("Karta sieciowa")
...
# Podpięcie NSG pod "karte sieciowa"
